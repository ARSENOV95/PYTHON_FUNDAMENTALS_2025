...
postivies = [filter(lambda x: x != None,list(num if num >= 0 else negatives.append(num) for num in number_sequence if num >= 0))]
even = [filter(lambda x: x != None,list(num if num %2 == 0 else odd.append(num) for num in number_sequence if abs(num) %2 == 0))]
